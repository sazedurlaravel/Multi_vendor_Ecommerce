...
class InstallerController extends Controller
{
    ...
    public function storeStep3(Request $request)
    {
        ...
        if(request()->hasFile("logo")){
            ...
            $filepath = "images/genral";
            $file_name = time().".".request()->file("logo")->getClientOriginalExtension();
            request()->file("logo")->move($filepath,$file_name);
            $newGenral->logo =  $file_name;
        }
        ...
        if(request()->hasFile("favicon")){
            if ($$newGenral->favicon != null && file_exists(public_path() . '/images/genral/' . $newGenral->favicon)) {
                unlink(public_path().'/images/genral/' . $newGenral->favicon);
            }
            $filepath = "images/genral";
            $file_name = time().".".request()->file("favicon")->getClientOriginalExtension();
            request()->file("favicon")->move($filepath,$file_name);
            $newGenral->favicon =  $file_name;
        }
    ...
    public function storeStep4(Request $request)
    {
        ...
        if(request()->hasFile("profile_photo")){
            if ($user->profile_photo != null && file_exists(public_path() . '/images/genral/' . $user->profile_photo)) {
                unlink(public_path().'/images/genral/' . $user->profile_photo);
            }
            $filepath = "images/user";
            $file_name = time().".".request()->file("profile_photo")->getClientOriginalExtension();
            request()->file("profile_photo")->move($filepath,$file_name);
